...
export default class DarkMode extends Component {
  state: { themeToggleDisplay: any; darkMode: boolean };
  props: any;
  setState: any;
  constructor(props) {
    super(props);

    let themeToggleDisplay: any = true;
    ...
    if (localStorage.getItem("theme-toggle-display")) {
      ...
      themeToggleDisplay = JSON.parse(
        localStorage.getItem("theme-toggle-display")
      );
    ...
    return (
      <div>
        ...
        <button
          ...
          type="button"
          className={
            (this.state.themeToggleDisplay ? "" : "hidden") +
            " text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm p-3 fixed bottom-2 left-2 z-30 opacity-40 hover:opacity-100"
          }
        >
          {/* light to dark icon */}
          <div
            className={(this.state.darkMode && "hidden") + " w-6 h-6"}
